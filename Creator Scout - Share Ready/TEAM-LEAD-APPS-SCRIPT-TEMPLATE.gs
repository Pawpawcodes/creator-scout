...
function doGet(e) {
  try {
    ...
    if (action === 'deleteCreator') {
      ...
      if (!profile_url) {
        return sendJSON({ error: 'Missing profile_url parameter', status: 'error', success: false });
      }

      const result = handleDeleteCreator(email, profile_url, personalSheetId);
      return sendJSON(result);
    }
    ...
    if (action === 'updateNotes') {
      ...
      const result = handleUpdateNotes(email, profile_url, notes, personalSheetId);
      return sendJSON(result);
    }
...
function handleGetCreatorStatus(email, profile_url) {
  ...
  try {
    ...
    if (!foundRow) {
      ...
      return { status: null, found: false, lock_in_price: null, notes: '' };
    }
    ...
    const notes = (data[lastRowIndex][8] || '').toString();

    console.log(`[GET STATUS] Returning: row=${foundRow}, status=${creatorStatus}, price=${lockInPrice}`);
    return { status: creatorStatus, found: true, lock_in_price: lockInPrice, notes: notes };
  } catch (error) {
    console.log(`[GET STATUS] Error: ${error.toString()}`);
    return { error: error.toString(), status: null, lock_in_price: null, notes: '' };
  }
}
...
function handleUpdateNotes(email, profile_url, notes, personalSheetId) {
  try {
    const { masterSheet } = ensureMasterSheets();
    const scoutId = getScoutId(email);

    if (!scoutId) {
      return { error: 'Scout not found', status: 'error', success: false };
    }

    // CRITICAL: Normalize URL for consistent lookup
    const normalizedUrl = normalizeProfileUrl(profile_url);
    console.log(`[UPDATE NOTES] Normalized URL: ${profile_url} → ${normalizedUrl}`);

    const masterData = masterSheet.getDataRange().getValues();
    let foundRow = null;

    // Find the LAST matching row (newest)
    for (let i = 1; i < masterData.length; i++) {
      const sheetUrl = normalizeProfileUrl(masterData[i][1]);
      if (masterData[i][0] === scoutId && sheetUrl === normalizedUrl) {
        foundRow = i + 1;  // Keep updating to get LAST match
    ...
    if (!foundRow) {
      console.log(`[UPDATE NOTES] Creator not found for scoutId=${scoutId}, url=${normalizedUrl}`);
      return { error: 'Creator not found', status: 'error', success: false };
    }

    // Update Notes column (column 9 - Master Sheet)
      ...
      try {
        ...
        if (personalMasterSheet) {
          ...
          for (let i = 1; i < personalData.length; i++) {
            ...
            if (personalData[i][0] === profile_url) {
              // Update Notes column (column 8 - Personal Sheet, column 9 - Master Sheet)
        ...
        Logger.log('Warning: Could not update notes in personal sheet: ' + error.toString());
      }
    }
...
function handleDeleteCreator(email, profile_url, personalSheetId) {
  try {
    const { masterSheet } = ensureMasterSheets();
    const scoutId = getScoutId(email);
    ...
    // CRITICAL: Normalize URL for consistent lookup
    const normalizedUrl = normalizeProfileUrl(profile_url);
    console.log(`[DELETE CREATOR] Normalized URL: ${profile_url} → ${normalizedUrl}`);

    // Delete from Master Sheet
    const deletedMasterRow = deleteRowByProfileUrl(masterSheet, scoutId, normalizedUrl);

    if (!deletedMasterRow) {
      console.log(`[DELETE CREATOR] Creator not found in Master Sheet for scoutId=${scoutId}, url=${normalizedUrl}`);
      return { error: 'Creator not found', status: 'error', success: false };
    }

    console.log(`[DELETE CREATOR] Deleted from Master Sheet at row ${deletedMasterRow}`);

    // Delete from personal sheet if provided
    if (personalSheetId) {
      try {
        const personalSheet = SpreadsheetApp.openById(personalSheetId);
        const personalMasterSheet = personalSheet.getSheetByName('Master');
        ...
        if (personalMasterSheet) {
          const deletedPersonalRow = deleteRowByProfileUrl(personalMasterSheet, null, profile_url, true);
          if (deletedPersonalRow) {
            console.log(`[DELETE CREATOR] Deleted from Personal Sheet at row ${deletedPersonalRow}`);
          }
        }
      } catch (error) {
        Logger.log('Warning: Could not delete from personal sheet: ' + error.toString());
        // Still return success since Master Sheet delete succeeded
      }
    }

    return { status: 'success', success: true, message: 'Creator deleted from both sheets' };
  } catch (error) {
    return { error: error.toString(), status: 'error', success: false };
  }
}

function deleteRowByProfileUrl(sheet, scoutId, normalizedUrl, isPersonalSheet = false) {
  try {
    const data = sheet.getDataRange().getValues();
    let rowsToDelete = [];

    if (isPersonalSheet) {
      // Personal sheet: Profile URL is column 1 (index 0), no Scout ID check needed
      for (let i = 1; i < data.length; i++) {
        if (data[i][0] === normalizedUrl) {
          rowsToDelete.push(i + 1);
        }
      }
    } else {
      // Master sheet: Scout ID is column 1 (index 0), Profile URL is column 2 (index 1)
      for (let i = 1; i < data.length; i++) {
        const sheetUrl = normalizeProfileUrl(data[i][1]);
        if (data[i][0] === scoutId && sheetUrl === normalizedUrl) {
          rowsToDelete.push(i + 1);
    ...
    // Delete rows in REVERSE order (highest to lowest) to maintain correct indices
    // This is critical for batch deletions to work correctly
    rowsToDelete.sort((a, b) => b - a);
    for (let i = 0; i < rowsToDelete.length; i++) {
      sheet.deleteRow(rowsToDelete[i]);
    }

    // Return the first deleted row number
